...
void poly_compress(unsigned char *r, const poly *a)
{
  ...
  for(i=0;i<AKCN_N;i+=8)
  {
    for(j=0;j<8;j++)
      t[j] = (((freeze(a->coeffs[i+j]) << 3) + AKCN_Q/2)/AKCN_Q) & 7;
...
void poly_decompress(poly *r, const unsigned char *a)
{
  ...
  for(i=0;i<AKCN_N;i+=8)
  {
    r->coeffs[i+0] =  (((a[0] & 7) * AKCN_Q) + 4)>> 3;
    r->coeffs[i+1] = ((((a[0] >> 3) & 7) * AKCN_Q)+ 4) >> 3;
    r->coeffs[i+2] = ((((a[0] >> 6) | ((a[1] << 2) & 4)) * AKCN_Q) + 4)>> 3;
    r->coeffs[i+3] = ((((a[1] >> 1) & 7) * AKCN_Q) + 4)>> 3;
    r->coeffs[i+4] = ((((a[1] >> 4) & 7) * AKCN_Q) + 4)>> 3;
    r->coeffs[i+5] = ((((a[1] >> 7) | ((a[2] << 1) & 6)) * AKCN_Q) + 4)>> 3;
    r->coeffs[i+6] = ((((a[2] >> 2) & 7) * AKCN_Q) + 4)>> 3;
    r->coeffs[i+7] = ((((a[2] >> 5)) * AKCN_Q) + 4)>> 3;
    a += 3;
  }
}
...
{
  unsigned char buf[AKCN_ETA*AKCN_N/4];
  unsigned char extseed[AKCN_SYMBYTES+1];
  int i;

  for(i=0;i<AKCN_SYMBYTES;i++)
    extseed[i] = seed[i];
  extseed[AKCN_SYMBYTES] = nonce;

  shake256(buf,AKCN_ETA*AKCN_N/4,extseed,AKCN_SYMBYTES+1);
...
void poly_sub(poly *r, const poly *a, const poly *b)
{
  int i;
  for(i=0;i<AKCN_N;i++)
    r->coeffs[i] = barrett_reduce(a->coeffs[i] + 3*AKCN_Q - b->coeffs[i]);
}
...
**************************************************/
void poly_frommsg(poly *r, const unsigned char msg[AKCN_SYMBYTES])
{
  uint16_t i,j,mask;
  ...
  for(i=0;i<AKCN_SYMBYTES;i++)
  {
    ...
    {
      mask = -((msg[i] >> j)&1);
      r->coeffs[8*i+j] = mask & ((AKCN_Q+1)/2);
    }
  }
}
...
*              - const poly *a:      pointer to input polynomial
**************************************************/
void poly_tomsg(unsigned char msg[AKCN_SYMBYTES], const poly *a)
{
  uint16_t t;
  int i,j;
  ...
  for(i=0;i<AKCN_SYMBYTES;i++)
  {
    ...
    for(j=0;j<8;j++)
    {
      t = (((freeze(a->coeffs[8*i+j]) << 1) + AKCN_Q/2)/AKCN_Q) & 1;
      msg[i] |= t << j;
    }
  }
}
