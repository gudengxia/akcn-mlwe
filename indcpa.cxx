...
static void pack_pk(unsigned char *r, const polyvec *pk, const unsigned char *seed)
{
  ...
  for(i=0;i<AKCN_SYMBYTES;i++)
    r[i+AKCN_POLYVECCOMPRESSEDBYTES] = seed[i];
}
...
static void unpack_pk(polyvec *pk, unsigned char *seed, const unsigned char *packedpk)
{
  ...
  for(i=0;i<AKCN_SYMBYTES;i++)
    seed[i] = packedpk[i+AKCN_POLYVECCOMPRESSEDBYTES];
}
...
static void pack_ciphertext(unsigned char *r, const polyvec *b, const poly *v)
{
  ...
  poly_compress(r+AKCN_POLYVECCOMPRESSEDBYTES, v);
}
...
static void unpack_ciphertext(polyvec *b, poly *v, const unsigned char *c)
{
  ...
  poly_decompress(v, c+AKCN_POLYVECCOMPRESSEDBYTES);
}
...
void gen_matrix(polyvec *a, const unsigned char *seed, int transposed) // Not static for benchmarking
{
  ...
  unsigned char extseed[AKCN_SYMBYTES+2];
  ...
  for(i=0;i<AKCN_K;i++)
  {
    for(j=0;j<AKCN_K;j++)
    {
      ...
      if(transposed)
      {
        extseed[AKCN_SYMBYTES]   = i;
        extseed[AKCN_SYMBYTES+1] = j;
      }
      else
      {
        extseed[AKCN_SYMBYTES]   = j;
        extseed[AKCN_SYMBYTES+1] = i;
      }

      shake128_absorb(state,extseed,AKCN_SYMBYTES+2);
...
*              public-key encryption scheme underlying AKCN
*
...
void indcpa_keypair(unsigned char *pk,
                   unsigned char *sk)
{
  polyvec a[AKCN_K], e, pkpv, skpv;
  unsigned char buf[AKCN_SYMBYTES+AKCN_SYMBYTES];
  unsigned char *publicseed = buf;
  unsigned char *noiseseed = buf+AKCN_SYMBYTES;
  int i;
  unsigned char nonce=0;

  randombytes(buf, AKCN_SYMBYTES);
  sha3_512(buf, buf, AKCN_SYMBYTES);
...
*              - const unsigned char *m:    pointer to input message (of length AKCN_SYMBYTES bytes)
...
{
  polyvec sp, pkpv, ep, at[AKCN_K], bp;
  poly v, k, epp;
