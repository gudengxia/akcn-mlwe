...
void ntt(uint16_t *p)
{
  ...
  {
    for(start = 0; start < AKCN_N; start = j + (1<<level))
    {
      zeta = zetas[k++];
      for(j = start; j < start + (1<<level); ++j)
      {
        ...
        p[j + (1<<level)] = barrett_reduce(p[j] + 4*AKCN_Q - t);
...
void invntt(uint16_t * a)
{
  ...
  for(level=0;level<8;level++)
  {
    ...
    {
      jTwiddle = 0;
      for(j=start;j<AKCN_N-1;j+=2*(1<<level))
      {
        ...
        t = (W * ((uint32_t)temp + 4*AKCN_Q - a[j + (1<<level)]));
